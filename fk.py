...
if st.button("Scrape"):
    ...
    sheet.append(["Product Name", "Price", "Rating and Review", "Product Link"])  # Header row
    ...
    while base_url and page_count < num_pages:
        ...
        for product in products:
            ...
            # Add product details to the Excel sheet
            sheet.cell(row=row, column=1, value=product_name)  # Product name
            sheet.cell(row=row, column=2, value=product_price)  # Price
            sheet.cell(row=row, column=3, value=rating_and_review)  # Rating and review
            sheet.cell(row=row, column=4, value=product_link)  # Product link
